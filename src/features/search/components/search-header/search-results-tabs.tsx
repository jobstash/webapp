...
export const SearchResultsTabs = ({
  ...
  return (
    <div className='flex min-h-0 flex-1 flex-col'>
      ...
      {activeGroup === 'jobs' && query.trim() && (
        <div className='border-b border-border px-2 py-2'>
          <Link
            href={`/?titleQuery=${encodeURIComponent(query.trim())}`}
            onClick={onItemSelect}
            className='flex w-full items-center justify-between gap-3 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none'
          >
            <span className='min-w-0 truncate'>
              View all jobs matching &ldquo;{query.trim()}&rdquo;
            </span>
