...
import { developerReportSchema, type DeveloperReport } from '../schemas';

const fetchDeveloperReportUncached =
  async (): Promise<DeveloperReport | null> => {
    try {
      const response = await fetch(
        `${clientEnv.MW_URL}/people/developer-report`,
        { cache: 'no-store' },
      );
